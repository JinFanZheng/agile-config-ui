...
> 状态：完成（G0 VERIFIED，2026-09-04 用户选型反馈落地）
> 变更分类：中改（改变全站视觉方向的关键决策，但本卡只产出静态稿与决策记录，不动生产代码）
> 方案出处：用户反馈「这个风格不太喜欢」（2026-09-04，针对 M0 已实现的 Linear 暗色风格）+ 明确指示「生成几个不同风格的静态网页，我选一下」
> 日期：2026-09-04 开始 / 2026-09-04 完成
> Goal 审查：PASS（见 1.3，GR-1..GR-10）
> 用户选型结论（2026-09-04 聊天，已回写 handoff §7.1）：**五风格全部保留为可切换主题；默认 石墨（C）；主布局定稿不再改**。落地为 ITER-02（主题系统）
...
| ---- | -------- | ------------------------------------------------------------ | ------------------------ | ----------------------------------------- | ------------------------------ | --------------- |
| G0   | VERIFIED | 用户已选定：五主题可切 + 默认石墨(C) + 布局定稿；已回写 §7.1 | 用户明确选定 + §7.1 回写 | 聊天记录（2026-09-04）+ handoff §7.1 diff | 无（迭代收口，后续见 ITER-02） | 无 / Codex      |
...
| RG-5 L-Real     | 用户真实查看并给出选择                    | 聊天记录：选定石墨(C)为默认 + 五主题全保留 + 布局定稿 | PASS |
...
- L-Real：http://localhost:5019/ 伺服；`docs/evidence/ITER-01/` 截图与检查输出；用户浏览后给出选型结论（见卡片头部）
- 副作用：无（静态文件、无 API 调用、无真实凭证）
- Goal 完成复审（2026-09-04）：G0 closure rule 满足（明确选择 + §7.1 已回写），迭代收口；产出方向移交 ITER-02
...
| ---- | ----------------------------------- | ------ | ------------------------------------------ | ------------------------ |
| L-01 | 选型后需新开迭代卡：src/ 主题化改造 | P2     | 已开 ITER-02 承接（本卡闭环条件不含实现）  | Codex / ITER-02          |
| L-02 | 静态稿是否长期保留在仓库            | P3     | 作为决策证据保留；ITER-02 完成后可评估移除 | Codex / ITER-02 收口时定 |
